      ...
      <CardHeader className="p-4 pb-2.5">
        <div className="flex items-start justify-between gap-3">
          <div>
            ...
            <CardDescription>Signals, validation, references, and math stay contextual.</CardDescription>
          </div>
          <Badge variant="secondary">Desk context</Badge>
        </div>
      </CardHeader>
      <CardContent className="p-4 pt-0">
        <Tabs value={activeTab} onValueChange={(value) => onTabChange(value as InsightTabId)}>
          <TabsList className="pg-tab-list-compact grid w-full grid-cols-4">
          ...
          <TabsContent value="signal" className="mt-3">
            <div className="space-y-3" data-testid="playground-advice-card">
              <div className="rounded-[18px] border p-3.5 shadow-sm">
                <div className="mb-2.5 flex items-center justify-between gap-3">
                  <div>
                    ...
                    <p className="text-[13px] leading-5 text-muted-foreground">Desk interpretation of the contract.</p>
                  </div>
                ...
                <div className={`rounded-[16px] border p-3 ${adviceTone(computed.advice?.type ?? "neutral")}`}>
                  <p className="mb-1.5 text-sm font-semibold">{computed.advice?.title ?? "Waiting for signal"}</p>
                  <p className="text-[13px] leading-5 text-slate-700">{computed.advice?.text ?? "Advice appears after the first price pass."}</p>
                </div>
              </div>

              {computed.surrogateSummary && (
                <div className="rounded-[18px] border border-border bg-white p-3.5 shadow-sm">
                  ...
                  <p className="text-[13px] leading-5 text-muted-foreground">{computed.surrogateSummary.message}</p>
                </div>
              )}

              <div className="rounded-[18px] border border-border bg-white p-3.5 shadow-sm" data-testid="playground-quickfacts-card">
                <div className="mb-3 flex items-center justify-between gap-3">
                  <div>
                    ...
                    <p className="text-[13px] leading-5 text-muted-foreground">Compact context for the contract.</p>
                  </div>
                ...
                <div className="space-y-2.5">
                  ...
                  <div className="grid gap-2.5 sm:grid-cols-2 xl:grid-cols-1">
                    {computed.quickFacts.map((fact) => (
                      <div key={fact.label} className="rounded-[16px] border border-border bg-slate-50/70 p-2.5">
                        <p className="mb-1.5 text-[11px] uppercase tracking-[0.14em] text-muted-foreground">{fact.label}</p>
          ...
          <TabsContent value="validation" className="mt-3">
            <div className="space-y-3" data-testid="playground-validation-card">
          ...
          <TabsContent value="reference" className="mt-3" data-testid="playground-references-card">
            <div className="rounded-[18px] border border-border bg-white shadow-sm">
              <div className="border-b border-border px-3.5 py-2.5">
                ...
                <p className="text-[13px] leading-5 text-muted-foreground">Methodology and implementation context for the active engine.</p>
              </div>
              <ScrollArea className="h-[360px] p-3.5">
                <div className="space-y-3 pr-3">
                  {computed.citations.map((citation, index) => (
                    <div key={citation.key} className="rounded-[16px] border border-border bg-slate-50/70 p-3">
                      <div className="mb-1.5 flex items-center gap-2">
                        <Badge variant="secondary">{index + 1}</Badge>
                        <p className="text-sm font-semibold">{citation.method}</p>
                      </div>
                      {citation.primary && <p className="text-[13px] leading-5 text-slate-700">{citation.primary}</p>}
                      {citation.secondary && <p className="mt-1 text-[13px] leading-5 text-muted-foreground">{citation.secondary}</p>}
                      {citation.textbook && <p className="mt-1.5 text-[13px] leading-5 text-slate-700">{citation.textbook}</p>}
                      {citation.implementationNote && <p className="mt-1.5 text-[13px] leading-5 text-muted-foreground">{citation.implementationNote}</p>}
                    </div>
                  ))}
                </div>
              </ScrollArea>
          ...
          <TabsContent value="math" className="mt-3">
            <div className="rounded-[18px] border border-border bg-white p-3.5 shadow-sm">
              <div className="mb-2.5">
                ...
                <p className="text-[13px] leading-5 text-muted-foreground">Keep formula details close, but out of the way until needed.</p>
              </div>
